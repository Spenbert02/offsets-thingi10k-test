...
def main():
    ...
    for subdir in mesh_dir_path.glob("model_*"):
        ...
        single_out_path = subdir / "singlebody_big" / f"model_{model_id}_singlebody_big_offset_output.msh"
        if single_out_path.exists():
            counts["success"] += 1
        
        twobody_out_path = subdir / "twobody_big" / f"model_{model_id}_twobody_big_offset_output.msh"
        if twobody_out_path.exists():
            counts['success'] += 1
    
    print(f"Offsets big progress: {counts['success']} / {counts['total']} successes")
