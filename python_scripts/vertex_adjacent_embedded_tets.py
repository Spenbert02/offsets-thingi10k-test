...
exactly two tetrahedra that are vertex-adjacent (share exactly one vertex
and no edge). Those two tets form the physical group/entity "tag_0"; every
other tet forms the physical group/entity "ambient". Neither tag_0 tet
touches the cube's boundary, not even at a single vertex.
...
The two tag_0 tets are the corner tets of two sub-cubes that touch only at
a single shared diagonal corner: the corner tet built at that shared point
from each cube has its right-angle vertex exactly there, so the pair meets
...
N = 5  # sub-cubes per axis; domain is the cube [0, N]^3

# Two sub-cubes that touch only at the single diagonal corner CUBE1 + (1,1,1)
# == CUBE2. CUBE1 must have even (i+j+k) parity (so its corner-tet set
# includes local corner (1,1,1)); CUBE2 is then automatically odd parity
# (so its corner-tet set includes local corner (0,0,0)).
CUBE1 = (1, 1, 2)
CUBE2 = (2, 2, 3)
...
def cube_tets(i, j, k):
    """Return (corner_set, corner_tets, central_tet)."""
    if (i + j + k) % 2 == 0:
    ...
    return corner_set, corner_tets, central
...
def build_mesh():
    assert tuple(a + 1 for a in CUBE1) == CUBE2, "CUBE2 must be CUBE1's diagonal neighbor"
    assert sum(CUBE1) % 2 == 0, "CUBE1 must have even parity"
    ...
    for i in range(N):
        for j in range(N):
            for k in range(N):
                corner_set, corner_tets, central = cube_tets(i, j, k)
                ...
                if pick_corner is not None:
                    idx = corner_set.index(pick_corner)
                    tag0_tets.append(global_corner_tets[idx])
                    ambient_tets.extend(
                        t for m, t in enumerate(global_corner_tets) if m != idx
                    )
...
def main():
    out_path = sys.argv[1] if len(sys.argv) > 1 else "vertex_adjacent_embedded_tets.msh"
