...
def main():
    ...
    for subdir in msh_dir_path.glob("model_*"):
        if subdir.exists() and subdir.is_dir():
            ...
            single_subdir = subdir / f"singlebody_big"
            single_subdir.mkdir(parents=True, exist_ok=True)
            single_json_path = single_subdir / f"singlebody_big_{model_id}_offset.json"
            single_data = {
                "application": "topological_offset",
                "input": str(input_msh_path),
                "output": f"model_{model_id}_singlebody_big_offset_output",
                "offset_tags": [ [0, 0] ],
                "offset_tag_val": [ [0, 2] ],
            ...
            twobody_subdir = subdir / f"twobody_big"
            twobody_subdir.mkdir(parents=True, exist_ok=True)
            twobody_json_path = twobody_subdir / f"twobody_big_{model_id}_offset.json"
            twobody_data = {
                "application": "topological_offset",
                "input": str(input_msh_path),
