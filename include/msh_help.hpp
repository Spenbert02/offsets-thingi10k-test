...
public:
    ...
    void add_empty_vertices(int dim)
    {
        if (dim < 1 || dim > 3)
        {
            throw std::invalid_argument("Only 1,2,3D elements are supported!");
        }

        if (m_spec.nodes.entity_blocks.empty())
        {
            throw std::invalid_argument("First vertex block cannot be empty.");
        }
    ...
    void add_physical_group(const std::string &name)
    {
        ...
        default:
            throw std::invalid_argument("Entity of dim {} cannot be written to .msh.");
        }
    }
        ...
        if (n_F > 0)
        {
            ...
            case 1:
                extract_simplex_elements<1>(
                    [&F](size_t i, size_t v0, size_t v1)
            ...
            case 3:
                ...
                break;
            default:
                std::string err_str = "Cannot extract elements for dimension" + std::to_string(dim);
...
private:
    ...
    void add_vertices(size_t num_vertices, const Fn &get_vertex_cb)
    {
        ...
        if (num_vertices == 0)
        {
            std::cout << "WARNING: Adding empty vertex block.";
        }
        mshio::NodeBlock block;
    ...
    void add_simplex_elements(size_t num_elements, const Fn &get_element_cb)
    {
        ...
        {
            throw std::invalid_argument("Please add a vertex block before adding elements.");
        }
        const auto &vertex_block = m_spec.nodes.entity_blocks.back();
        ...
        if (vertex_offset == 0)
        {
            std::cout << "WARNING: Do not offset vertex IDs for this element block as vertex block was empty";
        }
    ...
    void add_vertex_attribute(const std::string &name, const Fn &get_attribute_cb)
    {
        ...
        if (m_spec.nodes.entity_blocks.empty())
        {
            throw std::invalid_argument("Please add vertices before adding vertex attributes.");
        }
        const auto &vertex_block = m_spec.nodes.entity_blocks.back();
    ...
    void extract_vertex_attribute(const std::string &attr_name, Fn &&set_attr)
    {
        ...
        for (const auto &data : m_spec.node_data)
        {
            ...
            {
                throw std::invalid_argument("Attribute " + attr_name + " is of the wrong DIM.");
            }
