...
run_list_fpath = "/scratch/seb9449/offsets_testing_thingi10k/offsets-thingi10k-test/bash_scripts/manifold_array/pending_jobs.txt"
slurm_script_fpath = "/scratch/seb9449/offsets_testing_thingi10k/offsets-thingi10k-test/bash_scripts/manifold_array/manifold_submit_array.slurm"
RERUN_ALL = False
CHUNK_SIZE = 1000
...
def main():
    ...
    for subdir in mesh_dir_path.glob("model_*"):
        ...
        union_output_obj_path = subdir / "manifold_union" / f"model_{model_id}_manifold_union_output.obj"
        if union_output_obj_path.exists() and not RERUN_ALL:
            successes += 1
        else:
            union_json_path = subdir / "manifold_union" / f"manifold_union_{model_id}.json"
            if not union_json_path.exists():
                print(f"WARNING: manifold union json {str(union_json_path)} does not exist")
            else:
                jsons_to_run.append(str(union_json_path))
        
        subtract_output_obj_path = subdir / "manifold_subtract" / f"model_{model_id}_manifold_subtract_output.obj"
        if subtract_output_obj_path.exists() and not RERUN_ALL:
            successes += 1
        else:
            subtract_json_path = subdir / "manifold_subtract" / f"manifold_subtract_{model_id}.json"
            if not subtract_json_path.exists():
                print(f"WARNING: manifold subtract json {str(subtract_json_path)} does not exist")
            else:
                jsons_to_run.append(str(subtract_json_path))
    print(f"{successes} models already successfully offset.")
    run_list_path = Path(run_list_fpath)
    
    num_jobs = len(jsons_to_run)
    print(f"Found {num_jobs} manifold extraction jobs to run")
