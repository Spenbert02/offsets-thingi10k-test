...
run_list_fpath = "/scratch/seb9449/offsets_testing_thingi10k/offsets-thingi10k-test/bash_scripts/remeshing_test1_array/pending_jobs.txt"
slurm_script_fpath = "/scratch/seb9449/offsets_testing_thingi10k/offsets-thingi10k-test/bash_scripts/remeshing_test1_array/remeshing_test1_submit_array.slurm"
RERUN_ALL = False
CHUNK_SIZE = 1000
...
def main():
    ...
    for subdir in mesh_dir_path.glob("model_*"):
        ...
        output_msh_path = subdir / "remeshing_test1" / f"model_{model_id}_out.msh"
        if output_msh_path.exists() and not RERUN_ALL:
            successes += 1
        else:
            json_path = subdir / "remeshing_test1" / f"remeshing_test1_{model_id}.json"
            if not json_path.exists():
                print(f"WARNING: json {str(json_path)} does not exist")
            else:
                jsons_to_run.append(str(json_path))

    print(f"{successes} models already successfully ran through [remeshing_test1].")
    run_list_path = Path(run_list_fpath)
    
    num_jobs = len(jsons_to_run)
    print(f"Found {num_jobs} [remeshing_test1] jobs to run")
